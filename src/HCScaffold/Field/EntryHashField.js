...
// Class for creating the command arrays for fields of type EntryHash

class EntryHashField extends Field {
  constructor(field) {
    super(field);
  }
  ...
  get_commands(down_keystroke_counts) {
    ...
    this.create_yes_no_command(true, `field "${field.name}": Create link from EntryHash`);
    ...
    this.commands.push({
      ...
      "timeout": 1000,
      "comment": `field ${field.name}: choose entry type`
    })

    //this.create_return_command(`field ${field.name}: accept entry type ${field.entry_type}`);
    this.create_return_command(`field ${field.name}: accept default field name`);

    return this.commands;
  }
}

module.exports.EntryHashField = EntryHashField;
